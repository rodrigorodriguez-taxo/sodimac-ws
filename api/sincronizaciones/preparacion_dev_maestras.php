...
$rut    = trim($input['rut'] ?? '');
/*
 * Override de fecha SOLO para pruebas: environment.enviarVentanaLocal en la
 * app manda su propia "hoy" calculada en vez de dejar que este endpoint use
 * el reloj del servidor. Sirve para alinear la ventana operativa con agendas
 * de prueba ya sembradas en fechas fijas, sin tener que esperar al día
 * calendario real ni tocar la hora del dispositivo. Se ignora en silencio si
 * no llega o no tiene forma de fecha — nunca rompe el flujo normal.
 */
$fechaLocal = trim($input['fecha_local'] ?? '');
...
try {
    $data = prepararSincronizacion($pdo, $correo, $rut, $fechaLocal);
...
/* ============================================================================
   Flujo V3 — Contrato Sincronización Auditor V6 + jornadas (hoy y mañana)
   ============================================================================
   1. validar usuario          -> obtenerUsuario()
   2. tiendas autorizadas      -> obtenerTiendas()          (maestro)
   3. ventana operativa        -> [hoy, mañana], fecha del servidor
      Por cada fecha (OPERADOR — el analista solo mira hoy):
      3a. agenda del día         -> obtenerAgendas()          (V3.0)
          sin agenda para esa fecha => esa jornada se omite, no rompe la otra
      3b. productos/codigos      -> obtenerCodigosAgenda()    (V4.0 optimizado)
          4a. resolverAgendaMuestra()  (Paso A — autorizar + resolver id_muestra)
          4b. obtenerCodigosMuestra()  (Paso B — descarga por id_muestra)
      3c. agrupar por producto   -> agruparProductosConCodigos()
      3d. tienda de la agenda    -> tiendaDeAgenda() — NO current($tiendas):
          hoy y mañana pueden ser en locales distintos
   4. responder contrato APK: usuario, tiendas, jornadas[], zonas_tienda
   ============================================================================ */

function prepararSincronizacion(PDO $pdo, string $correo, string $rut, string $fechaLocal = ''): array
{
    /* --- 1. validar usuario ------------------------------------------------- */
     ...
     *
     * $fechaLocal la manda la app SOLO en desarrollo (ver el override arriba,
     * junto a donde se lee $input): reemplaza el reloj del servidor cuando
     * tiene forma de fecha, y si no, se cae al reloj real sin avisar.
     *
     * Ya NO se usa obtenerAgendasFallbackPrueba: era "la agenda operable más
     * reciente con fecha <= la pedida" — con mañana en la ventana, eso
     * devolvería la agenda de HOY disfrazada de la de mañana la primera vez
     * que mañana todavía no tenga agenda cargada.
     */
    $fechaHoy     = (preg_match('/^\d{4}-\d{2}-\d{2}$/', $fechaLocal) === 1) ? $fechaLocal : date('Y-m-d');
    $fechaManiana = date('Y-m-d', strtotime($fechaHoy . ' +1 day'));

    /* --- 4. separar por perfil ---------------------------------------------- */
    if (($usuario['tipo_usuario'] ?? '') === 'ANALISTA_CLIENTE') {
        return prepararSincronizacionAnalistaDev($pdo, $usuario, $tiendas, $fechaHoy);
    }

    return prepararSincronizacionOperadorDev($pdo, $usuario, $tiendas, [$fechaHoy, $fechaManiana]);
}

/*
 * La tienda de UNA agenda puntual, tal como vino en su propia fila.
 *
 * Antes se usaba current($tiendas) —la primera tienda del maestro del
 * operador— para las dos jornadas por igual. Con una sola jornada eso era
 * invisible; con hoy y mañana pudiendo ser en locales distintos, hubiera
 * dejado una de las dos con el sucursal_id equivocado. La fila de
 * obtenerAgendas ya trae id_tienda/codigo_tienda/nombre_tienda propios.
 */
function tiendaDeAgenda(array $agenda): array
{
    return [
        'id_tienda'     => (int) ($agenda['id_tienda'] ?? 0),
...
/* ============================================================================
   ...
   Contrato APK: usuario, tiendas, jornadas[], zonas_tienda.
   Cada jornada = un evento/agenda con sus muestras[] y cada muestra
   trae sus propios productos[].
   ============================================================================ */

function prepararSincronizacionOperadorDev(PDO $pdo, array $usuario, array $tiendas, array $fechas): array
{
    $login = $usuario['login'];

    /*
     * Una jornada sin agenda o sin muestras se omite sola: que mañana no
     * tenga nada cargado todavía no es error, es el resultado normal para
     * casi todos los días, y no puede tapar el trabajo de hoy.
     *
     * jornadas[] puede quedar vacío y ESO TAMPOCO ES ERROR: antes, sin agenda
     * para hoy, esto respondía un 401 que en la APK se confundía con
     * "credenciales inválidas". Ahora se informa con éxito y la lista vacía.
     */
    $jornadas = [];
    foreach ($fechas as $fecha) {
        ...
        /* Una fecha puede tener varias agendas: cada una es una jornada distinta */
        foreach ($agendas as $agenda) {
            $idAgenda = (int) $agenda['id_agenda'];

            /* Resolver todas las muestras activas de esta agenda */
            $muestrasAgenda = resolverAgendaMuestras($pdo, $idAgenda, $login);
            if (!$muestrasAgenda) {
                continue;
            }

            $muestras = [];
            foreach ($muestrasAgenda as $muestraRow) {
                $idMuestra = (int) $muestraRow['id_muestra'];

                /* Descargar productos de esta muestra */
                $filasCodigos = obtenerCodigosMuestra($pdo, $idMuestra);
                if (!$filasCodigos) {
                    continue;
                }

                /* Inyectar metadata de agenda en cada fila para agruparProductosConCodigos */
                foreach ($filasCodigos as &$fila) {
                    $fila['id_agenda']       = $agenda['id_agenda'];
                    $fila['numero_agenda']   = $agenda['numero_agenda'];
                    $fila['fecha_agenda']    = $agenda['fecha_agenda'];
                    $fila['id_tienda']       = $agenda['id_tienda'];
                    $fila['id_muestra']      = $muestraRow['id_muestra'];
                    $fila['codigo_muestra']  = $muestraRow['codigo_muestra'] ?? null;
                    $fila['nombre_muestra']  = $muestraRow['nombre_muestra'] ?? null;
                }
                unset($fila);

                $productos = agruparProductosConCodigos($filasCodigos);

                $muestras[] = construirMuestraConProductos($muestraRow, $agenda, $productos);
            }

            /* Sólo agregar jornada si tiene al menos una muestra con productos */
            if (empty($muestras)) {
                continue;
            }

            $jornadas[] = [
                'evento'  => construirEventoDesdeAgenda($agenda, tiendaDeAgenda($agenda)),
                'muestras' => $muestras,
            ];
        }
    }
...
function prepararSincronizacionAnalistaDev(PDO $pdo, array $usuario, array $tiendas, string $fecha): array
{
    $login = $usuario['login'];

    /*
     * --- 3b. agenda del día (sin fallback, sin ventana de mañana) ------------
     * El analista sigue con una sola jornada, la de hoy — la ventana de dos
     * días es del operador, que es quien cuenta.
     */
    $agendas = obtenerAgendas($pdo, $login, $fecha);
    if (!$agendas) {
    ...
    $eventos = construirEventoDesdeAgenda($agendaSeleccionada, tiendaDeAgenda($agendaSeleccionada));
    ...
    /*
     * El analista no usa jornadas[] para nada operativo —su dato real está en
     * 'analista' más abajo— pero el parser del front espera el mismo sobre
     * para los dos perfiles. Una sola entrada, sin muestra ni productos.
     */
    return [
        'usuario'       => $usuario,
        'tiendas'       => $tiendas,
...
/* ============================================================================
   4-A2. resolverAgendaMuestras  (equivale a resolverAgendaMuestra pero
   devuelve TODAS las muestras activas de la agenda, sin LIMIT 1)
   Parametros: :id_agenda, :login
   ============================================================================ */

function resolverAgendaMuestras(PDO $pdo, int $idAgenda, string $login): ?array
{
    $sql = "SELECT
        a.id_agenda,
        a.numero_agenda,
...
/* ============================================================================
   4-C. construirMuestraConProductos
   Arma el objeto muestra[] que la APK espera dentro de jornada.muestras[].
   ============================================================================ */

function construirMuestraConProductos(array $muestraRow, array $agenda, array $productos): array
{
    return [
        'id_muestra'     => (int) ($muestraRow['id_muestra'] ?? 0),
        'codigo_muestra' => ((($muestraRow['codigo_muestra'] ?? '') !== '' && (int)($agenda['id_agenda'] ?? 0) > 0)
                            ? ($muestraRow['codigo_muestra'] . '-AG' . (int)$agenda['id_agenda'])
                            : ($muestraRow['codigo_muestra'] ?? '')),
        'nombre_muestra' => $muestraRow['nombre_muestra'] ?? '',
