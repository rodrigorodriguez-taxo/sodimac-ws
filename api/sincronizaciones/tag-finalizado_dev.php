...
foreach ($detalles as $idx => $detalle) {
    ...
    // Validar lecturas[]
    $lecturas = $detalle['lecturas'] ?? null;
    if (!is_array($lecturas) || count($lecturas) === 0) {
        errorResponse("Detalle[$idx]: Falta lecturas[] o esta vacio");
    }
    ...
    foreach ($lecturas as $lIdx => $lectura) {
        ...
        $fechaHora  = trim($lectura['fecha_hora'] ?? '');

        if ($lecturaUid === '') {
            errorResponse("Detalle[$idx].lecturas[$lIdx]: Falta lectura_uid");
        }
        if ($cantidad === null || !is_numeric($cantidad)) {
            errorResponse("Detalle[$idx].lecturas[$lIdx]: Falta cantidad o valor invalido");
        }
        if ($fechaHora === '') {
            errorResponse("Detalle[$idx].lecturas[$lIdx]: Falta fecha_hora");
        }

        $sumaLecturas += (float)$cantidad;
        $totalLecturas++;
    }

    // Validar consistencia: suma de lecturas debe coincidir con cantidad_fisica
    if (abs($sumaLecturas - (float)$cantidadFisica) > 0.001) {
        errorResponse("Detalle[$idx]: suma de lecturas ($sumaLecturas) no coincide con cantidad_fisica ($cantidadFisica)");
    }
}

// ──────────── Calcular totales desde lecturas ────────────
$totalProductos = count($detalles);
$totalUnidades  = 0;
foreach ($detalles as $detalle) {
    foreach ($detalle['lecturas'] as $lectura) {
        $totalUnidades += (int)$lectura['cantidad'];
    }
}
...
try {
    ...
    // Upsert detalle por lectura individual en sod_pda_tag_carga_detalle
    $stmtDetalle = $pdo->prepare("
        INSERT INTO sod_pda_tag_carga_detalle (
            carga_id, lectura_uid, sku, codigo_barras, descripcion,
    ...
    foreach ($detalles as $detalle) {
        ...
        foreach ($detalle['lecturas'] as $lectura) {
            ...
            $stmtDetalle->execute([
                ...
                ':stock_sistema'   => 0,
                ':cantidad_fisica' => $cantidadLectura,
                ':diferencia'      => 0,
                ':fecha_hora'      => $fechaHoraLectura,
            ]);
        }
    }
    ...
    if ($iteracion === 1) {
        ...
        if ($todosOk) {
            ...
            foreach ($resultadosSgo as $r) {
                $resultado = strtoupper(trim($r['resultado'] ?? ''));
                if (!in_array($resultado, ['INSERTADO', 'DUPLICADO_IGNORADO'], true) || empty($r['id_conteo_det']) || empty($r['id_tag'])) {
                    $errores[] = ($r['lectura_uid'] ?? $r['detalle_uid'] ?? '?') . ': ' . ($r['mensaje'] ?? $r['resultado'] ?? 'ERROR');
                }
            }
            $errorMsg = 'SP rechazo lecturas: ' . implode('; ', $errores);
            ...
            errorResponse('SP rechazo una o mas lecturas: ' . $errorMsg, 422);
        }
    }
